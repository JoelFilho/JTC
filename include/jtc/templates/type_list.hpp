...
//     static_assert(is_same_v<found_type, char>, "Should be char.");
//-------------------------------------------------------------------------------------------------
...
template <typename Node, typename... Nodes, size_t Index>
struct list_get<type_list<Node, Nodes...>, Index>
    : conditional<Index == 0, Node, list_get_t<type_list<Nodes...>, Index - 1>> {};
...
// Item Search
//
// Returns a result_type that indicates the first match where is_same_v<Node, Query>() == true
//
// Usage:
//     using my_list = type_list<char, short, int>;
...
//     static_assert(is_same_v<list_get_t<other_list, 0>, false_type>(), "...");
//-------------------------------------------------------------------------------------------------
...
struct find_param_type : conditional_t<is_same_v<Query, Node>(),              // Has the type been found?
...
struct find_param_type<Query, Node> : result_type<is_same_v<Query, Node>(), Node> {};
...
struct find_param_operator : conditional_t<BoolOperator<Node>::value,                        // Does Node match the operator?
